...
\end{verbatim}
The \verb;Raw; terms use de Bruijn indexing, hence they have natural
numbers in the \verb;var; constructor. Additionally, the lambda terms
are annotated so that type inference is possible. The checked
\verb;Term;'s are indexed by \verb;CtxTp;, which is simply the
non-dependent pair (\verb;Prod;, derived from \verb;Sigma;) of a
context (\verb;Ctx;, which is a \verb;List; of types \verb;Tp;)
and a type (\verb;Tp;). The indexed \verb;ivar; constructor contains a
membership proof (\verb;Mem;, a standard list membership relation),
ensuring that the type \verb;A; appears in the context \verb;G;.
