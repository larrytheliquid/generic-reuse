...
%% Title information
%% \title{Generic Zero-Cost Reuse of Dependently Typed\\ Programs, Proofs, and Datatypes}         %% [Short Title] is optional;
\title{Generic Zero-Cost Reuse for Dependent Types}
%% \title{Generic Zero-Cost Reuse of Dependently Typed Programs}
...
that reusing functions involves linear time conversions between
differently indexed types, resulting in a
runtime \textit{performance penalty} incurred by practicing the good software
engineering practice of code reuse. In this paper we address both of
...
\item{\refsec{rel}: We compare what we have done with related
...
(covered in \refsec{back:cdle}).\footnote{
  \raggedright{The Cedille code accompanying this paper is here:\\
...
As an example of Curry-style data, consider the standard definitions of
Church-encoded lists and vectors below:
\begin{verbatim}
...
It is not possible to reuse function of type \verb;AppL; to define
...
indices.\footnote{
  ...
  sorted (as discussed in \refsec{future}).
  }
...
type is isomorphic to the \verb;Id; typed introduced by \citet{firsov18b}.
...
our related work, \refsec{rel:judg}).
...
Note that \verb;appV2appL;, above, simultaneously captures the linear
time conversion function and the extensional identity proof
...
\item The identity functor law.
\item The composition functor law.
\end{enumerate}
...
``efficient'' lambda-encoding (using Mendler-style F-algebras), in the
...
eliminators.\footnote{
  There is no predecessor problem to worry about when deriving schemes
  with induction principles (or, eliminators), because schemes
  contain no inductive occurrences.
}
...
\begin{verbatim}
imapL ◂ ∀ A : ★. IdMapping (ListF A) = λ f. elimListF
  (pair nilLF β)
  (λ x,xs. pair (consLF x (elimId f xs) β)).

imapV ◂ ∀ A : ★. IIdMapping Nat (VecF A) = λ f. elimVecF
  (pair nilVF β)
  (λ x,xs. pair (consVF x (elimId f xs) β)).
\end{verbatim}
The returned value is \verb;Sigma;-type the codomain of \verb;Id; from
...
interesting subterm is the tail argument (\verb;elimId f xs;) of the
...
\textit{identity} function being mapped (\verb;f;) is erased when
zero-cost converting (i.e. \verb;|elimId f xs| = xs;).
Hence, \verb;β; is evidence of the
cons rebuilding cases because \verb;|consLF x (elimId f xs)| = |consLF| x xs;,
and \verb;|consVF x (elimId f xs)| = |consVF| x xs;.
...
lf2vf ◂ ∀ A : ★. ∀ X : Nat ➔ ★. ∀ Y : ★. 
 Π f : Y ➔ Nat. 
 IdDep Y (λ y. X (f y)) ➔ 
 IdDep (ListF A Y ) (λ xs. VecF A X (lenAlgM f xs))
  = λ c. elimListF
  (pair nilVF β)
  (λ x,xs. pair (consVF x (elimIdDep c xs) β)).
 
l2v ◂ ∀ A : ★. IdDep (List A) (λ xs. Vec A (len xs))
  = fix2ifix imapV imapL lenAlgM lf2vf.
\end{verbatim}
...
%% \section{Generic Refined Reuse}
%% \labsec{ref}

%% \section{Dependent Premise Enrichment}
%% %% or Relational Remember Enrichment or Relational Enriching Reuse?
%% \labsec{dep}
...
judgement, with \verb;IdDep; a corresponding dependent version
