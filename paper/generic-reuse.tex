...
Our work has 2 notable limitations:
\begin{enumerate}
\item We can only perform zero-cost coercions when indexed types
  store their index data as \textit{erased} arguments
  (or more accurately, when the erased and non-erased arguments of
  non-indexed and indexed types match). This may not
  always be desirable, and has consequences such as needing to
  recompute the length of a vector if it is needed dynamically.
\item Forgetul program (or proof) reuse is not possible when the domain of the
  indexed function is constrained, such as the vector head function, by using
...
\begin{verbatim}
IdDep Fix RawF imapR (λ t. ∀ GA : CtxTp. Typed GA t ➾ IFix CtxTp TermF imapT)
...
as part of the premise) and the non-dependent data.
...
erased for the zero-cost coercion to indeed be an indentity function
...
in various places for the pair of the context and type
...
arguments we must supply to get an identity coercion back from
\verb;c2;, which functions as an inductive hypothesis would for the
purposes of this combinator. We explicitly supply both of these
...
Note that dependent interoperability and zero-cost conversions solve
related, but different, problems. Consider the \verb;headV; function
for vectors, \verb;headV ◂ ∀ A : ★. ∀ n : Nat. Vec A (suc n) ➔ A;.
